...
<body style="font-family: verdana,sans-serif" BGCOLOR="#819FF7">
  ...
  <hr>
  <p>
  <input type="button" class="button" value="Calibrate MPU" onclick="button_clicked('cal', 'MPU Calibrating...')"> 
  <input type="button" class="button" value="Calibrate Compass" onclick="button_clicked('cal_compass', 'Compass Calibrating...')"> 
  <input type="button" class="button" value="Reset Calibration" onclick="button_clicked('reset_cal', 'Resetting...')"> 
  </p>
  ...
  <script>
    ...
    function button_clicked(key, message) { 
      var status = document.getElementById("status");
      status.innerText = message || "Processing...";
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
            // Wenn MPU-Kalibrierung fertig ist (blockierend), Status aktualisieren
            if (key === "cal") {
                status.innerText = "Fertig!";
                setTimeout(function() { status.innerText = ""; }, 2000);
            } else if (key === "cal_compass") {
                // Bei Kompass-Kalibrierung nicht sofort "Fertig!" setzen, 
                // da dies über requestData() gesteuert wird
                status.innerText = "Kalibriert...";
            } else {
                status.innerText = ""; 
            }
        }
    ...
    var lastCalibratingState = false;
    var pollInterval = 1000; // Standard Intervall

    function requestData() {
      var xhr = new XMLHttpRequest();
      xhr.open('GET', 'get_settings?t=' + new Date().getTime(), true);
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          if (xhr.responseText) {
            ...
            // DEBUG: Hier sehen wir in der Browser-Konsole (F12), was der ESP sendet
            console.log("ESP Data:", data);
            console.log("isCalibrating Status:", data.isCalibrating);
            
            // Einstellungen aktualisieren
            if (data.azimut !== undefined) Azimut.value = data.azimut;
            if (data.elevation !== undefined) Elevation.value = data.elevation;
            if (data.el_offset !== undefined) El_offset.value = data.el_offset ;
            if (data.az_offset !== undefined) Az_offset.value = data.az_offset ;
            if (data.motor_speed !== undefined) Motor_Speed.value = data.motor_speed ;
            ...
            if (data.isCalibrating) {
               status.innerText = "Kalibriert...";
               lastCalibratingState = true;
               pollInterval = 200; // Schneller abfragen
            } else if (lastCalibratingState === true) {
               status.innerText = "Fertig!";
               lastCalibratingState = false;
               pollInterval = 1000; // Wieder langsamer
               setTimeout(function() { status.innerText = ""; }, 2000);
            }
          }
        }
